                ...
                if (settings.readWeeks.length > 0) { // If there's data for which weeks were read, then use the more complex process
                    for (i = 0; i < numberOfWeeks; i += 1) {
                        if ((startIncrementDateObj.getMonth() % 2) === 0) { // remainder function to find even month
                            row = row + "<tr class='cal-row' data-date=\"" + createDateString(startIncrementDateObj) + "\">";
                    ...
                    for (i = 0; i < numberOfWeeks; i += 1) {
                        if ((startIncrementDateObj.getMonth() % 2) === 0) { // remainder function to find even month
                            row = row + "<tr class='cal-row' data-date=\"" + createDateString(startIncrementDateObj) + "\"><td class=\"cal-month cal-even-month " + rowHeightClass + "\">";
                            if (startIncrementDateObj.getDate() < 8) {
                                row = row + monthText(startIncrementDateObj, settings.monthNames) + "</td>";
                            } else {
                                row = row + "&nbsp;</td>";
                            ...
                            row = row + "<tr class='cal-row' data-date=\"" + createDateString(startIncrementDateObj) + "\"><td class=\"cal-month cal-odd-month " + rowHeightClass + "\">";
                            if (startIncrementDateObj.getDate() < 8) {
                                row = row + monthText(startIncrementDateObj, settings.monthNames) + "</td>";
                            } else {
                                row = row + "&nbsp;</td>";
