    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-md-flex align-items-center justify-content-between">
                <h4 class="mb-0 font-size-18"><span class="mr-3">Brochure Listing</span>
                    @can('brochure_manage')
                        <a href="{{ route('brochure_add') }}" class="btn btn-outline-success waves-effect waves-light btn-sm">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <div class="col">
                            <form method="POST" action="{{ route('brochure_listing') }}">
                                @csrf
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group custom-dropdown-container">
                                            <label for="brochure_status">Status</label>
                                            {!! Form::select('brochure_status', $status, @$search['brochure_status'], [
                                                'class' => 'form-control select2',
                                                'id' => 'brochure_status',
                                            ]) !!}
                                        </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-nowrap">
                            ...
                            <tbody>
                                <?php
                                $no = $brochures->firstItem();
                                ?>
                                @if ($brochures->isNotEmpty())
                                    @foreach ($brochures as $brochure)
                                        <?php
                                        if ($brochure->is_published) {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="flex-shrink-0 mr-4">
                                                    @if ($brochure->hasMedia('brochure_media'))
                                                        <img src="{{ $brochure->getMedia('brochure_media')->last()?->getUrl('thumb') }}"
                                                        height="70" width="100" class="brochure-thumbnail-clickable"
                                            ...
                                            <td style="max-width: 300px; word-wrap: break-word; white-space: normal;">
                                                <div class="d-flex">
                                                    <div class="flex-grow-1">
                                                        @php
                                                            $data = json_decode($brochure->car_model);
                                                            $brand = $brochure->company->car_brand->car_brand_name;
                                            ...
                                            @if ($user_type == 'administrator')
                                                <td>
                                                    {{ $brochure->company?->company_name }}
                                                    <small>({{ @$brochure->company->sector?->sector_name }})</small>
                                                </td>
                                            @endif
                                            @if (Auth::user()->roles->value('id') == 3)
                                                <td style="max-width: 400px; word-wrap: break-word; white-space: normal;">
                                                    @php
                                                        $branch_ids = $brochure->company_branch_id;
                                                        $company_branch = $brochure->company->company_branch->where('is_deleted', 0);
                                                        $branch = [];
    ...
    <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <form method="POST" action="{{ route('brochure_status') }}">
                    @csrf
                    <div class="modal-body">
                        <h4>Delete this brochure ?</h4>
...
@section('script')
    ...
    <script>
        ...
        var images = document.querySelectorAll(".brochure-thumbnail-clickable");
