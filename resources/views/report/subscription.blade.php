...
@section('title') Subscription Reporting @endsection
...
<div class="row">
	<div class="col-12">
		<div class="page-title-box d-flex align-items-center justify-content-between">
			<h4 class="mb-0 font-size-18">Subscription Reporting</h4>
							...
							<div class="row">
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										{!!Form::select('filter_by', $filter_by_sel , @$search['filter_by'], ['class' => 'form-control select_active'])!!}
									</div>
								</div>
								<div class="col-md-3">
									<div class="form-group">
								...
								<div class="col-md-3">
									<div class="form-group">
										<label>Status</label>
										{!! Form::select('status_id', $status_sel, @$search['status_id'], ['class' => 'form-control select_active']) !!}
									</div>
								</div>
							</div>
							<div class="row">
                    ...
                    <table id="datatable" class="table table-bordered w-100 display nowrap">
                        <thead id="table-header" class="thead-light">
                            <tr align="center">
								...
								<th>Billing Month</th>
								<th>Subscription Remaining Month</th>
								<th>Subscription Total Amount</th>
								<th>Subscription Tax Amount</th>
								<th>Amount Paid</th>
								<th>Payment Type</th>
								<th>Payment Date</th>
						...
						@if(@$records)
							@foreach($records as $row)
                                <tr align="center">
									...
									<td>{{ $row->subscription_package_price ?? '-'}}</td>
									<td>{{ $row->billing_month ?? '-'}}</td>
									<td>{{ $row->remaining_month ?? '-'}}</td>
									<td>{{ $row->subscription_total_amount ?? '-'}}</td>
									<td>{{ $row->subscription_tax_amount ?? '-'}}</td>
									...
									<td>{{ $row->setting_coverage_area_name ?? '-'}}</td>
