                    ...
                    <div class="row">
                        ...
                        @else
                            @if($action == 'edit')
                                <div class="col-sm-6">
							        <label for="company_id">Company <span class="text-danger">*</span></label>
                                    <input name="type" id="type" type="text" class="form-control select2" value="{{ @$product_category->company->company_name }}" readonly>
						        </div>
                            @else
                                <div class="col-sm-6">
							        <label for="company_id">Company <span class="text-danger">*</span></label>
								    ...
								    @foreach($company as $key => $val)
									    <option value="{{$key}}" {{ $key == @$post->company_id ? 'selected' : ($key == (@$product_category->company_id ?? null) ? 'selected' : '') }}>{{$val}}</option>
								    @endforeach
							        </select>
						        </div>
                            @endif
                    ...
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input id="product_category_name" type="text" name="product_category_name" class="form-control" value="{{ @$post->product_category_name }}">
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="product_category_priority">Product Category Priority </label>
                                <input id="product_category_priority" type="number" name="product_category_priority" class="form-control" value="{{@$post->product_category_priority }}">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="product_category_status">Publish </label>
                                {!! Form::select('is_published', $status, @$post->is_published ?? '0', ['class' => 'form-control select2', 'id' => 'product_category_status' ]) !!}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-6">
                            ...
                            <a href="{{ route('product_category_listing') }}" class="btn btn-secondary" type="button">Cancel</a>
                        </div>
                    </div>
                </div>
            </div>
...
@section('script')
    ...
    <script>
        function updateImageCountAndPreviews(input) {
            ...
            imageCountDisplay.textContent = `${imageCount} ${imageCount === 1 ? 'image' : 'images'} selected`;
        ...
        function updateImageCountAndPreviews2(input) {
            const imageCount = input.files.length;
            const imageCountDisplay = document.getElementById("imageCount2");
            imageCountDisplay.textContent = `${imageCount} ${imageCount === 1 ? 'image' : 'images'} selected`;
